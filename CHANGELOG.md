...
- **Line breaks after layout / grid change:** `TwDoc.resize` replays `fullText()` with `putc`, which **ignores** newline characters (they are not printable ASCII in `putc`). Newlines in the serialized text are now applied with `newline()`, and import/resize replay forces **typeover** so insert mode does not shift rows. Without this, the first layout after `load` collapsed all lines into one. ([BR-004](docs/bug-reports/BR-004-resize-dropped-newlines-collapsed-lines.md))
